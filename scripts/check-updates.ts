#!/usr/bin/env tsx
/**
 * Ukrainian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://zakon.rada.gov.ua';
const PORTAL_NAME = 'Zakon (Verkhovna Rada)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/ukrainian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Ukrainian Law MCP — Data Freshness Check');
