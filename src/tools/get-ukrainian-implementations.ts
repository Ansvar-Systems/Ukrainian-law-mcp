/**
 * get_ukrainian_implementations — Find Ukrainian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetUkrainianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface UkrainianImplementationResult {
  document_id: string;
...
export async function getUkrainianImplementations(
  db: InstanceType<typeof Database>,
  input: GetUkrainianImplementationsInput,
): Promise<ToolResponse<UkrainianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as UkrainianImplementationResult[];
