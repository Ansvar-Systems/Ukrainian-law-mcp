/**
 * search_eu_implementations — Search EU directives/regulations referenced by Ukrainian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_ukrainian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  ukrainian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as ukrainian_statute_count
  ...
  if (input.has_ukrainian_implementation) {
    sql += ' HAVING ukrainian_statute_count > 0';
  }
