/**
 * Tool registry for Ukrainian Law MCP Server.
...
import { getUkrainianImplementations, type GetUkrainianImplementationsInput } from './get-ukrainian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Verkhovna Rada Official Portal. ' +
    ...
    'Call this FIRST when you need to understand what Ukrainian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Ukrainian statute. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Ukrainian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Ukrainian?" ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Ukrainian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Ukrainian statute references or aligns with. ' +
      'Ukrainian is an EU candidate state aligning its legislation with the EU acquis ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Ukrainian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_ukrainian_implementations',
    description:
      'Find all Ukrainian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Ukrainian statutes. ' +
      'Ukrainian transposes EU directives into national law as an EU candidate state.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Ukrainian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_ukrainian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Ukrainian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Ukrainian statute. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_ukrainian_implementations':
          result = await getUkrainianImplementations(db, args as unknown as GetUkrainianImplementationsInput);
          break;
